...
// var OrderCollect = require('../../proxy').OrderCollect;
...
var index = function (req, res, next) {
  ...
  ep.all('orders', function (orders) {
    orders.forEach(function (order) {
      UserModel.findById(order.author_id, ep.done(function (author) {
        if (mdrender) {
          order.content = renderHelper.markdown(at.linkUsers(order.content));
        }
        order.author = _.pick(author, ['loginname', 'avatar_url']);
        ep.emit('author');
      }));
    });

    ep.after('author', orders.length, function () {
      orders = orders.map(function (order) {
        return _.pick(order, ['id', 'author_id', 'tab', 'content', 'title', 'last_reply_at',
